...
describe('JobsService', () => {
  ...
  describe('findAll', () => {
    ...
    it('should mark jobs as applied when user has applied', async () => {
      // Arrange
      const userId = 1;
      const jobs = [{ id: 1, name: 'Software Developer', postedById: 2 }];
    ...
    it('should mark jobs as not applicable when user is the poster', async () => {
      // Arrange
      const userId = 2;
      const jobs = [{ id: 1, name: 'Software Developer', postedById: userId }];
  ...
  describe('createApplication', () => {
    it('should throw ConflictException when user already applied to the job', async () => {
      // Arrange
      const applicantId = 1;
      ...
      const existingApplication = { id: 1, jobId: 2, applicantId: 1 };
      ...
      // Act & Assert
      await expect(
        service.createApplication(applicantId, createApplicationDto),
    ...
    it('should throw NotFoundException when job not found for acceptApplication', async () => {
      // Arrange
      const userId = 1;
      const jobId = 2;
      const applicantId = 3;
      ...
      // Act & Assert
      await expect(
        service.acceptApplication(userId, jobId, applicantId),
      ).rejects.toThrow(NotFoundException);
    });
